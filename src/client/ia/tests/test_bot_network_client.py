...
Run from project root:
    sharp_blaze_venv\\Scripts\\python.exe -m pytest src/client/ia/tests/test_bot_network_client.py -v
    
Or standalone:
    sharp_blaze_venv\\Scripts\\python.exe src/client/ia/tests/test_bot_network_client.py
"""
...
class MockTCPServer:
    ...
    - Reads the INITIAL_CONNECT message
    - Can send responses back
    """
    def __init__(self):
        self.server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        self.server_socket.bind(("127.0.0.1", 0))  # OS picks port
    ...
    def read_message(self, timeout=2.0) -> dict:
        """Read one newline-delimited JSON message from the client."""
...
class MockUDPServer:
    ...
    - Sends back 12-byte position packets
    """
    def __init__(self):
    ...
    def receive_hello(self, timeout=3.0) -> tuple:
        """Receive and parse a hello message. Returns (session_id, player_id, checksum, addr)."""
        self.server_socket.settimeout(timeout)
    ...
    def send_position(self, entity_id: int, grid_x: float, grid_y: float, addr=None):
        """Send a 12-byte position packet to the client."""
...
def test_tcp_connect_and_initial_message():
    """Test 1: TCP connection sends correct INITIAL_CONNECT format."""
    print("\n--- Test 1: TCP Connect + INITIAL_CONNECT ---")
    
    server = MockTCPServer()
    bot = BotNetworkClient()

    # Connect in background (connect_tcp blocks briefly)
    ...
    # Accept the connection on mock server
    server.accept_one()
    ...
    # Verify INITIAL_CONNECT message format
    ...
    print(f"  ✓ INITIAL_CONNECT format correct: {json.dumps(msg, indent=2)}")
...
def test_tcp_send_json():
    """Test 2: send_json sends newline-delimited JSON."""
    ...
    t.start()
    server.accept_one()
    server.read_message()  # consume INITIAL_CONNECT
    t.join(timeout=3)

    # Send a BUY_UNIT command
    buy_cmd = {
        "type": "BUY_UNIT",
    ...
    # Read it on mock server
    msg = server.read_message()
    assert msg["type"] == "BUY_UNIT"
    assert msg["payload"]["unit_type"] == "Attacker"
    print(f"  ✓ BUY_UNIT sent correctly: {msg}")

    # Send a MOVE_ORDER command
    move_cmd = {
        "type": "MOVE_ORDER",
    ...
    print(f"  ✓ MOVE_ORDER sent correctly: {msg}")
...
def test_tcp_receive_json():
    """Test 3: receive_json parses newline-delimited JSON from server."""
    ...
    # Server sends a BUY_UNIT_RESULT
    server.send_message({
    ...
    assert msg["payload"]["unit_id"] == 1000
    print(f"  ✓ BUY_UNIT_RESULT received correctly: {msg}")

    # Server sends multiple messages at once (batched)
    ...
    print(f"  ✓ Batched messages received: {msg1['type']}, {msg2['type']}")
...
def test_udp_hello_format():
    """Test 4: UDP hello message has correct 12-byte binary format."""
    print("\n--- Test 4: UDP Hello Format ---")
    
    udp_server = MockUDPServer()
    bot = BotNetworkClient()

    # Run init_udp in background (it has retry loops)
    t = threading.Thread(
        target=bot.init_udp,
    ...
    print(f"  ✓ Hello format correct: session={session_id}, player={player_id}, checksum={checksum}")

    # Stop the init loop by simulating server response
...
def test_udp_position_reception():
    """Test 5: UDP position packets are received and converted correctly."""
    ...
    # Send a few position packets from the mock server
    # Entity 1000 (attacker) at grid (5, 90)
    udp_server.send_position(1000, 5.0, 90.0, addr)
    # Entity 3002 (collector) at grid (10, 85)
    udp_server.send_position(3002, 10.0, 85.0, addr)
    # Entity 6000 (enemy attacker) at grid (90, 5)
    ...
    # Verify grid-to-world conversion: world = (grid * 50) + 25
    # Entity 1000: grid(5, 90) → world(275, 4525)
...
def test_grid_to_world():
    """Test 6: Grid-to-world conversion matches NetworkManager."""
    ...
    test_cases = [
        ...
        (5.0, 90.0, 275, 4525),       # Player 1 area
        (90.0, 5.0, 4525, 275),       # Player 2 area
        (50.0, 50.0, 2525, 2525),     # Center of map
        (99.0, 99.0, 4975, 4975),     # Bottom-right corner
    ]
    ...
    for grid_x, grid_y, exp_x, exp_y in test_cases:
        ...
        print(f"  ✓ grid({grid_x},{grid_y}) → world({wx},{wy})")

    bot.disconnect()


def test_disconnect_cleanup():
    """Test 7: Disconnect properly cleans up all state."""
    ...
    print("  ✓ All state cleaned up after disconnect")

    bot.disconnect()  # Double disconnect should not crash
    print("  ✓ Double disconnect is safe")
