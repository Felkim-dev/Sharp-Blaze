...
Prerequisites:
- Docker container 'sharp-blaze-test' must be running on ports 5555/5556.
"""
...
def run_e2e_test():
    ...
    # Connection params matching the 'docker run' command
    IP = "127.0.0.1"
    TCP_PORT = 5555
    UDP_PORT = 5556
    SESSION_ID = 100
    ...
    try:
        ...
        while time.time() - start_time < 5:
            if not ack1:
                ...
                if m1 and m1.get("type") == "CONNECTION_ACK":
                    ack1 = True
                    print("  ✓ P1 Connected & Acked")
            if not ack2:
                m2 = client2.receive_json()
                if m2 and m2.get("type") == "CONNECTION_ACK":
                    ack2 = True
                    print("  ✓ P2 Connected & Acked")
            if ack1 and ack2: break
            time.sleep(0.1)

        # 4. Send START_GAME command (from P1)
        ...
        # Poll for messages (up to 15 seconds)
        start_time = time.time()
        while time.time() - start_time < 15:
            ...
            if received_start1 and received_start2:
                print("  ✓ Both clients received START_GAME!")
                break
            time.sleep(0.1)

        if not (received_start1 and received_start2):
            print(f"  ✗ Timeout! Start1={received_start1}, Start2={received_start2}")
            return

        # 3. Initialize UDP
        print("\n[Step 4] Initializing UDP for both...")
        client1.init_udp(IP, UDP_PORT, SESSION_ID, 1) # Player 1
        client2.init_udp(IP, UDP_PORT, SESSION_ID, 2) # Player 2

        print("[Step 5] Waiting for UDP positions (5s)...")
        # Buy a unit via Client 1 to trigger some activity if needed
        # (Actually, server spawns initial structures/units on START_GAME usually)
        ...
        while time.time() - start_time < 5:
            ...
            if pos1:
                print(f"  ✓ Player 1 received {len(pos1)} entity positions via UDP")
                pos_received = True
            if pos2:
                print(f"  ✓ Player 2 received {len(pos2)} entity positions via UDP")
                pos_received = True
